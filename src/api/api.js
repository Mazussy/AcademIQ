...
function extractToken(loginResponse) {
  ...
  if (loginResponse.data && loginResponse.data.accessToken) return loginResponse.data.accessToken;
  if (loginResponse.result && loginResponse.result.token) return loginResponse.result.token;
...
export async function login(email, password) {
  ...
  for (let i = 0; i < variants.length; i++) {
    ...
    try {
      ...
      if (!err || (err.status && (err.status === 500 || err.status === 400 || err.status === 415))) {
        continue;
      }
...
export async function logout() {
  try { await http.post('/api/identity/Account/Logout'); } catch { /* ignore */ }
  setToken(null);
}
...
export const studentApi = {
  ...
  notifications: async () => http.get('/api/customer/Student/GetStudentNotifications'),
  // Attendance endpoint needs confirmation; temporarily align with user note
  attendance: async () => http.get('/api/customer/Student/GetStudentNotifications'),
};

// Enrollment (Student)
export const enrollmentApi = {
  availableOfferings: async () => http.get('/api/customer/Enrollment/GetAllCourseOfferingsForStudent'),
  myEnrollments: async () => http.get('/api/customer/Enrollment/GetAllEnrollments'),
  registerCourse: async (id) => http.post(`/api/customer/Enrollment/RegisterCourse/${id}`),
  deleteRegisteredCourse: async (id) => http.del(`/api/customer/Enrollment/DeleteRegisterdCourse/${id}`),
};

// Instructor
export const instructorApi = {
  info: async () => http.get('/api/customer/Instructor/InstructorInfo'),
  scheduling: async () => http.get('/api/customer/Instructor/SchedulingOfInstructor'),
  allCourses: async () => http.get('/api/customer/Instructor/AllCourses'),
  studentsRegistered: async (id) => http.get(`/api/customer/Instructor/AllStudentRegistred/${id}`),
  markAttendance: async (enrollmentId, status) => http.post('/api/customer/Instructor/MarkAttendance', { enrollmentId, status }),
  sendNotification: async ({ student_Id, course_Id, type_Alert, message }) =>
    http.post('/api/customer/Instructor/SendNotification', { student_Id, course_Id, type_Alert, message }),
};
